...
public class InstanceSynchronizer {
    ...
    public void removeIfMissing(Set<String> currentSet, Class ofType) {
        String idListKey = ID_LIST_KEY_PREFIX + ofType + ID_LIST_KEY_POSTFIX;
        ...
        if (oldSetObject == null) {
            logger.info("Record created for " + ofType.getSimpleName() + " with KEY: " + idListKey);
        ...
        if (oldSet.size() > 0) {
            logger.info("There is " + oldSet.size() + " of " + ofType.getCanonicalName() + " to remove from db");
